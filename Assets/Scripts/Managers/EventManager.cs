...
class EventManager : ManagerBase
{
    ...
    //所有事件的id和事件的缓冲池，key为事件id，value为事件本身
    ...
    //注册事件
    public void Register(int id, UnityEvent eventMgr)
    {

        if (EventListerDict.ContainsKey(id))//如果注册过这个事件
        {
            Debug.LogError("Key:" + id + "已存在！");//告诉你这个事件已经注册过
        }
        else//如果没有注册
        {
            if (eventMgr == null)
            ...
            EventListerDict.Add(id, eventMgr);
            Debug.Log("Key" + id + "注册成功！");//事件注册成功
        }
    }
